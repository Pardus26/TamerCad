export class STLBinaryWriter {
    write(body, _options = {}) {
        ...
        for (const triangle of mesh.getTriangles()) {
            ...
            for (const index of triangle.getVertexIndices()) {
                const vertex = mesh.getVertex(index).position;
    ...
    computeNormal(mesh, triangle) {
        const ids = triangle.getVertexIndices();
        const p0 = mesh.getVertex(ids[0]).position;
        const p1 = mesh.getVertex(ids[1]).position;
        const p2 = mesh.getVertex(ids[2]).position;
        const ux = p1.x - p0.x;
        const uy = p1.y - p0.y;
        const uz = p1.z - p0.z;
        const vx = p2.x - p0.x;
...
//# sourceMappingURL=STLBinaryWriter.js.map