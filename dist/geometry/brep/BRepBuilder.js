...
export class BRepBuilder {
    ...
    static fromBox(box) {
        ...
        */
        for (const surface of box.getSurfaces()) {
            const loop = new BRepLoop();
            const face = new BRepFace(surface, loop);
            shell.addFace(face);
        }
     ...
     * Cylinder → BRepSolid
     */
    static fromCylinder(cylinder) {
        const solid = new BRepSolid();
        const shell = new BRepShell();
        for (const surface of cylinder.getSurfaces()) {
            const loop = new BRepLoop();
            const face = new BRepFace(surface, loop);
            shell.addFace(face);
        }
    ...
    static fromSphere(sphere) {
        const solid = new BRepSolid();
        const shell = new BRepShell();
        for (const surface of sphere.getSurfaces()) {
            const loop = new BRepLoop();
            const face = new BRepFace(surface, loop);
            shell.addFace(face);
        }
     ...
     */
    static build(solid) {
        if (solid instanceof BoxSolid3) {
            return this.fromBox(solid);
        }
        if (solid instanceof CylinderSolid3) {
            return this.fromCylinder(solid);
        }
        if (solid instanceof SphereSolid3) {
            return this.fromSphere(solid);
        }
     ...
     * Vertex oluşturucu
     */
    static createVertex(point) {
        return new BRepVertex(point);
    }
    /**
     * Edge oluşturucu
     */
    static createEdge(start, end, curve) {
        return new BRepEdge(start, end, curve);
    }
    /**
     * Boş BRep solid
     */
