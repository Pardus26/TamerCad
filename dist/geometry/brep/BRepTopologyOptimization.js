export class BRepTopologyOptimization {
    ...
    calculateSensitivity() {
        for (const element of this.elements) {
            element.sensitivity =
                -Math.pow(element.density, this.options.penalty);
        }
    }
    /**
     * Sensitivity filter
     ...
     */
    updateDensity() {
        const target = this.volume.target;
        for (const element of this.elements) {
            element.density =
                Math.max(0.001, Math.min(1, element.density *
                    target));
        }
    }
    /**
     ...
     */
    generateGeometry() {
        const removed = this.elements.filter(e => e.density <
            0.1);
        return {
    ...
    info() {
        return {
            engine: "BRepTopologyOptimization",
            elements: this.elements.length,
            status: "READY"
        };
    }
}
//# sourceMappingURL=BRepTopologyOptimization.js.map