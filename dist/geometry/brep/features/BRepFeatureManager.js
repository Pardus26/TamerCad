...
export class BRepFeatureManager {
    ...
    updateFeature(request) {
        ...
        const before = feature.serialize();
        const parameter = feature.parameters.find(p => p.name === request.parameter);
        if (parameter) {
            parameter.value =
                request.value;
        }
        this.constraints.propagateChange(request.parameter, request.value);
        this.solver.solve();
        this.history.recordChange({
            id: crypto.randomUUID(),
            featureId: feature.id,
    ...
    rebuild() {
        ...
        const validation = this.validator.validateFeatures(Array.from(this.features.values()));
        return {
     ...
     */
    snapshot() {
        return this.history.createSnapshot(Array.from(this.features.values()));
    }
    /**
     * Undo
     */
    ...
    optimize() {
        ...
        return this.optimizer.optimize("PERFORMANCE");
    }
    /**
     * Feature getir
     */
    ...
    serialize() {
        return {
            features: this.getFeatures().map(f => f.serialize()),
            history: this.history.serialize(),
            constraints: this.constraints.serialize()
        };
    }
    /**
     * Reset
    ...
    info() {
        return {
            engine: "BRepFeatureManager",
            features: this.features.size,
            history: this.history.changes.length
        };
    }
}
//# sourceMappingURL=BRepFeatureManager.js.map