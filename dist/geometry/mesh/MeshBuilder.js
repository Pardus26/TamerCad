    ...
    build(triangles, options = {}) {
        ...
        for (const triangle of triangles) {
            ...
            for (const p of triangle.vertices) {
                const key = weld
                    ? this.makeKey(p[0], p[1], p[2], tolerance)
                    : `${vertexId}`;
                let index = vertexMap.get(key);
                if (index === undefined) {
                    index = mesh.addVertex(new MeshVertex(vertexId++, new Point3(p[0], p[1], p[2])));
                    vertexMap.set(key, index);
                }
                indices.push(index);
            }
            const face = new MeshTriangle(triangleId++, indices[0], indices[1], indices[2]);
            mesh.addTriangle(face);
        }
        return mesh;
    }
    makeKey(x, y, z, tolerance) {
