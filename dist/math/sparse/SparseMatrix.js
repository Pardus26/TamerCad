...
export class SparseMatrix {
    ...
    static fromDense(matrix) {
        const sparse = new SparseMatrix(matrix.rows, matrix.cols);
        let count = 0;
    ...
    multiply(vector) {
        const result = Vector.zeros(this.rows);
        for (let row = 0; row < this.rows; row++) {
            ...
            for (let i = start; i < end; i++) {
                sum +=
                    this.values[i] *
                        vector.get(this.columnIndices[i]);
            }
            result.set(row, sum);
        }
        return result;
    ...
    toDense() {
        const dense = Matrix.zeros(this.rows, this.cols);
        for (let row = 0; row < this.rows; row++) {
            const start = this.rowPointers[row];
            const end = this.rowPointers[row + 1];
            for (let i = start; i < end; i++) {
                dense.set(row, this.columnIndices[i], this.values[i]);
            }
        }
        return dense;
    }
